...
from persona.target_driven_planning import main_plan
...
class ReverieServer:
    def __init__(self,
        ...
        if not resume or not os.path.exists(sim_folder):
        ...
        with open(f"{fs_temp_storage}/curr_step_backup.json", "w") as outfile:
            outfile.write(json.dumps(curr_step, indent=2))
    ...
    """
        ...
        while (True):
            ...
            if not waiting_flag:
                ...
                print(f"current step remaining: {int_counter}")
    ...
    def task_module_setup(self, task, llm_model):
        performer_name = task['performer']
        performer_persona = None
        for persona_name, persona in self.personas.items():
            if performer_name == persona_name:
                performer_persona = persona
                break

        planner = main_plan.TargetDrivenPlanner(task, performer_persona, model=llm_model)
        planner.initial_plan()

        set_var('task_planner', planner)
        set_var('performer_name', performer_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--task_id', type=int, default=None)
    parser.add_argument('-r', '--resume', action='store_true', default=False)
    parser.add_argument('-p', '--resume_env')
    parser.add_argument('--naive', action='store_true', default=False)
    parser.add_argument('-m', '--llm_model', default='gpt-35-turbo')
    parser.add_argument('--model_path')
    parser.add_argument('-a', '--appendix', default='')

    opt = parser.parse_args()
    llm_model = opt.llm_model
    appendix = opt.appendix

    if llm_model == 'gpt4':
        utils.build_gpt4()
    elif 'local' in llm_model or 'llama' in llm_model:
        utils.build_llama(opt.model_path)

    task_json = json.load(open(opt.task_file))
    if opt.task_id is None:
        task_id = input(f"Must enter the id of the task, from 0 to {len(task_json - 1)}").strip()
    else:
        task_id = opt.task_id

    task = task_json[task_id]

    origin = task['env_name']
    if not opt.resume:
        target = origin.replace('base', 'sim') + '_' + datetime.datetime.now().strftime("%m%d_%H%M") + '_' + llm_model + appendix
    else:
        target = opt.resume_env

    dump_data_dir = os.path.join('data', 'dumped_data', target + '_' + llm_model)
    os.makedirs(dump_data_dir, exist_ok=True)

    set_var("task", task)
    set_var("dump_data_dir", dump_data_dir)
    performer_name = task['performer']
    set_var('performer_name', performer_name)
    set_var('performer_llm_model', llm_model)
    ...
    rs = ReverieServer(origin, target, opt.resume)
    if not opt.naive:
        rs.task_module_setup(task, llm_model)
    rs.open_server()
