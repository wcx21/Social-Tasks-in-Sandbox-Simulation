...
def run_gpt_prompt_generate_hourly_schedule(persona,
    ...
    def create_prompt_input(persona,
        ...
        # for count, i in enumerate(persona.scratch.daily_plan_req):
...
def run_gpt_prompt_task_decomp(persona,
    ...
    def __func_clean_up(gpt_response, prompt=""):
        ...
        # WCX: sometimes GPT gives unexpected note, we ignore it
        # temp = [i.strip() for count, i in enumerate(gpt_response.split("\n")) if count == 0 or ')' in i]
        temp = [i.strip() for count, i in enumerate(gpt_response.split("\n")) if (count == 0 or ')' in i) and 'duration' in i]
        ...
        for count, i in enumerate(_cr):
            ...
            # wcx: to fix the format error, the error is caused by too long time and too many subtasks. 23.10.23
            # k = [j.strip() for j in re.split("\(\s*duration in minutes*\s*:", i)]
            k = [j.strip() for j in re.split("\(\s*duration in minut*es*\s*:", i)]
            task = k[0]
            if task[-1] == ".":
                task = task[:-1]
            try:
    ...
    gpt_param = {"engine": "text-davinci-003", "max_tokens": 1500,
                 ...
                 "frequency_penalty": 0.2, "presence_penalty": 0, "stop": None}
...
def run_gpt_prompt_action_sector(action_description,
    ...
    def create_prompt_input(action_description, persona, maze, test_input=None):
        ...
        for i in curr:
            ...
            # else:
            #     fin_accessible_sectors += [i]
            # commented by wcx on 2023.11.24 to enable visiting
            fin_accessible_sectors += [i]
...
def run_gpt_prompt_action_arena(action_description,
    ...
    ## enable original protect code
    y = f"{act_world}:{act_sector}"
    x = [i.strip() for i in persona.s_mem.get_str_accessible_sector_arenas(y).split(",")]
    if output not in x:
        new_output = random.choice(x)
        print(f"protect triggered! output: {output} --to-- {new_output}")
        output = new_output
    ## end protect
...
def run_gpt_prompt_summarize_conversation(persona, conversation, test_input=None, verbose=False):
    ...
    # ChatGPT Plugin ===========================================================
    def __chat_func_clean_up(gpt_response, prompt=""):  ############
        # modified by wcx 23.10.13 to avoid duplicated "conversing about"
        resp = gpt_response.strip()

        for suffix in ['this is a conversation about ', 'This is a conversation about ']:
            if resp.startswith(suffix):
                resp = resp.replace(suffix, '', 1)

        if not resp.startswith("conversing about "):
            ret = "conversing about " + resp
        else:
            ret = resp
        return ret
    ...
    #              "temperature": 0, "top_p": 1, "stream": False,
    #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    gpt_param = {"engine": "text-davinci-002", "max_tokens": 60,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    ...
    # example_output = "conversing about dating at supermarket tomorrow 14:00 pm"  ########
    example_output = "conversing about the course project. " \
                     "<Character A> promises to help <Character B> with the programming project tomorrow in the library at 11 am."  ########
    ...
    special_instruction = "The output must continue the sentence above by filling in the <fill in> tag. Don't start with" \
                          " 'this is a conversation about...' Just finish the sentence but do not miss any important" \
                          " details (if there is an appointment, include the characters, specific time and location)."  ########
...
def run_gpt_generate_iterative_chat_utt(maze, init_persona, target_persona, retrieved, curr_context, curr_chat,
    ...
    def create_prompt_input(maze, init_persona, target_persona, retrieved, curr_context, curr_chat, test_input=None):
        ...
        # special for ga tasks
        # if persona.scratch.name == 'Arthur Burton':
        #     special_instruction = 'Important note: As Arthur Burton, you must remember your purpose is inviting more ' \
        #                           'people to the wine party at The Rose and Crown Pub at 5:00 pm Tuesday February ' \
        #                           '14th. To do this, you may need to make clear invitation and make sure others are ' \
        #                           'clear with the exact time and location.'
        # else:
        #     special_instruction = ''
        special_instruction = ''
    ...
    def __chat_func_validate(gpt_response, prompt=""):
        ...
        try:
            ...
            # print (gpt_response)
            # print ("debug 2")
            assert gpt_response is not None and extract_first_json_dict(gpt_response) is not None, "response is None"
    ...
    gpt_param = {"engine": engine, "max_tokens": 400,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
