...
def generate_first_daily_plan(persona, wake_up_hour): 
  ...
  # if performer_name is None or performer_name != persona.name:
  #   daily_plan = run_gpt_prompt_daily_plan(persona, wake_up_hour)[0]
  # else:
  #   daily_plan = task_planner.gen_new_daily_req(persona, wake_up_hour)[0]

  if task_planner is not None and performer_name == persona.name:
    daily_plan = task_planner.gen_new_daily_req(persona, wake_up_hour)[0]
  else:
    if persona.name == performer_name and get_var("performer_llm_model") is not None:
      llm_model = get_var("performer_llm_model")
    else:
      llm_model = 'gpt-35-turbo'
    daily_plan = run_gpt_prompt_daily_plan(persona, wake_up_hour, engine=llm_model)[0]
...
def generate_hourly_schedule(persona, wake_up_hour): 
  ...
  for i in n_m1_activity:
    # if i != prev:
    # modified by wcx to avoid syntax error, truncated by 3 hours
    if i != prev or (prev_count >= 3 and i != 'sleeping'):
      prev_count = 1
...
def revise_identity(persona): 
  ...
  # moved here -- by WCX
  new_daily_req_list = new_daily_req.split('\n')
  new_daily_req_list = [dr.strip().strip('.') for dr in new_daily_req_list]
  new_daily_req_list = [dr.replace(f"{i}.", '').strip() for i, dr in enumerate(new_daily_req_list)]
  persona.scratch.daily_req = new_daily_req_list
...
def _long_term_planning(persona, new_day): 
  ...
  task = get_var('task')
  if task is not None and new_day:
    performer = task['performer']
    if performer == persona.name:
      dump_data_dir = get_var('dump_data_dir')
      planning_dump_dir = os.path.join(dump_data_dir, 'planning')
      os.makedirs(planning_dump_dir, exist_ok=True)
      planning_file = os.path.join(planning_dump_dir,
                                f'{datetime.datetime.now().strftime("%m%d_%H%M%S")}_{performer}.pkl')
      data = [task, persona, new_day]
      pickle.dump(data, open(planning_file, 'wb'))
  ...
  if new_day == "First day": 
    ...
    # IDK what happened but it seems should be daily_plan_req  --Wcx
